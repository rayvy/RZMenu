...
void main(uint3 dispatchThreadID : SV_DispatchThreadID)
{
    ...
    // Глобальная координата, куда будем писать
    ...
    if (EnableMask > 0.5f)
    {
        // Читаем маску. Используем localCoord, так как маска обычно обрезана под размер региона
        mask_value = MaskTexture.Load(int3(localCoord, 0)).r;
        
        // !!! ФИКС БАГА !!!
        // Если маска черная (прозрачная), мы выходим из функции.
        // Мы НЕ пишем в Атлас, оставляя то, что там было (предыдущие слои).
        if (mask_value <= 0.001f)
            return;
    }

    // --- ЧТЕНИЕ ИСТОЧНИКА ---
    // ВАЖНО: Читаем исходную текстуру (Source) из тех же координат, куда пишем (targetPixelCoord).
    // Это нужно, если SourceTexture полноразмерная (лицо целиком).
    // Если твоя SourceTexture это маленький кусочек, верни localCoord.
    float4 base_color = SourceTexture.Load(int3(targetPixelCoord, 0));
    ...
    if (mask_value > 0.001f)
    {
        ...
        // --- СМЕШИВАНИЕ (BLENDING) ---
        // Если маска полупрозрачная (края, антиалиасинг), нам нужно смешать
        // Новый цвет с тем, что УЖЕ ЕСТЬ на Атласе (предыдущий слой).
        // Читаем текущий пиксель атласа для фона:
        float3 background_color = Atlas[targetPixelCoord].rgb;
        
        // Смешиваем: (Фон * (1-mask)) + (НовыйЦвет * mask)
        final_rgb = lerp(background_color, modified_rgb, mask_value);
    }
