...
Texture2D<float4> tex_overlay_mask : register(t42);
#define value IniParams[72].x
...
void main(vs2ps input, out float4 result : SV_Target0)
{
    ...
    float4 mask        = tex_overlay_mask.Load(int3(pixelCoord, 0));

    float maskValue = value + mask.r;
    float threshold = 1.0;

	float blendAmount = (maskValue > threshold) ? 1.0 : 0.0;
	if(value > 0.99){blendAmount = 1;}
	if(value < 0.01){blendAmount = 0;}
	if(overlay.w > 0) {
	result.rgb = lerp(baseColor.rgb, overlay.rgb, 1.0 - blendAmount);
