# Vendored from gret-1_4_0/patcher.py.
# Thanks to the gret authors for the PanelPatcher/FunctionWrapper approach.
# Local RZMenu changes are intentionally small: remove gret prefs/log imports and
# default panel patching to enabled so the helper can run standalone in RZMenu.
...
class FunctionWrapper:
    ...
    def __enter__(self):
        ...
        if not module:
            ...
            print(f"Couldn't patch {module_name}.{self.function_name}, module not found")
            return None

        base_function = self.get_function(module, self.function_name)
        if not base_function:
            print(f"Couldn't patch {module.__name__}.{self.function_name}, function not found")
            return None
