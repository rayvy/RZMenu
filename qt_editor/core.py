...
# --- WRITE ---
# (Функции create_element, delete_elements и т.д. оставляем без изменений, 
#  так как они пишут данные, но update_property_multi нужен для флагов)
...
def update_property_multi(target_ids, prop_name, value, sub_index=None, fast_mode=False):
    ...
    try:
        ...
        # Mapping UI names to Blender names
        map_props = {
            "pos_x": ("position", 0),
            "pos_y": ("position", 1),
            "width": ("size", 0),
            ...
            "color": ("color", None) # Требует спец обработки
        }
        ...
        for elem in elements:
            if elem.id in target_ids:
                if hasattr(elem, bl_prop):
                    ...
                    if bl_prop == "color":
                        # Value is [r,g,b,a] list
                        # Blender color is IDPropertyArray or similar
                        try:
                            # Простая установка массива
...
def reorder_elements(target_id, insert_after_id):
    ...
    try:
        ...
        # 1. Смена родителя (Drag & Drop в Outliner часто меняет иерархию)
        # Если мы перетащили НА элемент -> ставим parent_id
        # Если между -> parent_id берем от insert_after_id
        
        # STUB: Для этой фазы пока реализуем только линейную сортировку в коллекции
        # В фазе 2 нужно добавить логику parent_id
        ...
        if insert_after_id is None:
            ...
            to_index = anchor_idx if target_idx < anchor_idx else anchor_idx + 1 # Blender collection move quirk
...
def toggle_editor_flag(target_ids, flag_name):
    """
    Toggles a boolean flag (is_hidden, is_locked, is_selectable)
    """
    map_flags = {
        "is_hidden": "qt_hide",
        "is_locked": "qt_locked",
...
def reparent_element(child_id, new_parent_id):
    ...
    try:
        ...
        if target:
            ...
            # TODO: Convert Local/Global coords here?
