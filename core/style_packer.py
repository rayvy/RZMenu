...
def pack_styles(scene, export_dir):
    ...
    Layout:
    - Index 0: Reserved empty style (all zeros). Shader uses styleId=0 as "no style".
    - Index 1+: Actual user-defined styles (mapped with +1 offset from Blender style_id).
    """
    ...
    def pack_empty_style(buf):
        """Write 12 float4 slots of zeros. This is the 'no style' sentinel at index 0."""
        buf.extend(struct.pack('<Ifff', 0, 0.0, 0.0, 0.0))  # Slot 0: flags = 0
        for _ in range(11):
            buf.extend(struct.pack('<ffff', 0.0, 0.0, 0.0, 0.0))

    # --- Index 0: always reserved as "no style" ---
    pack_empty_style(style_buffer)

    # --- Index 1+: actual user styles ---
    for style in styles:
        flags = 0
