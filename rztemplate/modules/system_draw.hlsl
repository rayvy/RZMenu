...
// --- Smooth notification Y-position interpolation factor ---
float get_y_factor(float age) {
    float x = age / 5.0;
    float idx = floor(x);
    float f = frac(x);
    float t = smoothstep(0.96, 1.0, f); // transition over the last 0.2s of the 5s interval
    return idx + t;
}
...
float4 main(VSOut input) : SV_Target {
    ...
    // Status Block coordinates (Vertical compact layout)
    float status_right = main_left - 15.0;
    float STATUS_W = 160.0;
    float status_left = status_right - STATUS_W;

    float firstOff_mode = 0.0;
    float width_mode = GetTextWidth(Slot2, len_mode, firstOff_mode) * scale_M;
    float mode_x = status_left + (STATUS_W - width_mode) / 2.0;
    float mode_y = 20.0 - BG_PAD + 2.0;

    // D-pad center
    float2 ac = float2(status_left + STATUS_W / 2.0, y_top_limit - 14.0);

    // Capture indicator dot center
    float2 cc = float2(mode_x - 10.0, mode_y + H_M / 2.0);

    // ----------------------------------------------------------------
    // 3. TEXT RENDERING PASS (Highest Priority)
    // ----------------------------------------------------------------

    // Macro for proportional baseline-aligned text rendering
    ...
    { \
        ...
        if (rel.y >= -10.0 * scale_val && rel.y < 35.0 * scale_val) { \
    ...
    DRAW_TEXT_LINE(Slot2, len_mode, mode_x, mode_y, scale_M, float4(0.85, 0.85, 0.85, 1.0), 1.0)
    ...
    if (Notice0Time > 0.0 && age0 >= 0.0 && age0 < 8.0) {
        float x_left = lerp(ScreenRes.x, ScreenRes.x - BOX_W - 20.0, saturate(age0 / 0.4) * saturate(age0 / 0.4) * (3.0 - 2.0 * saturate(age0 / 0.4)));
        float y_bottom = 20.0 + H_S + 5.0 + H_L + BG_PAD + 15.0 + get_y_factor(age0) * (BOX_H + 10.0);
        uint len_n0 = 0; Slot3.GetDimensions(len_n0);
        DRAW_TEXT_LINE(Slot3, len_n0, x_left + 15.0, y_bottom + (BOX_H - H_S) / 2.0, scale_S, float4(0.95, 0.95, 0.95, 1.0), saturate((8.0 - age0) / 2.0))
    }

    // Notice 1
    float age1 = Time - Notice1Time;
    if (Notice1Time > 0.0 && age1 >= 0.0 && age1 < 8.0) {
        float x_left = lerp(ScreenRes.x, ScreenRes.x - BOX_W - 20.0, saturate(age1 / 0.4) * saturate(age1 / 0.4) * (3.0 - 2.0 * saturate(age1 / 0.4)));
        float y_bottom = 20.0 + H_S + 5.0 + H_L + BG_PAD + 15.0 + get_y_factor(age1) * (BOX_H + 10.0);
        uint len_n1 = 0; Slot4.GetDimensions(len_n1);
        DRAW_TEXT_LINE(Slot4, len_n1, x_left + 15.0, y_bottom + (BOX_H - H_S) / 2.0, scale_S, float4(0.95, 0.95, 0.95, 1.0), saturate((8.0 - age1) / 2.0))
    }

    // Notice 2
    float age2 = Time - Notice2Time;
    if (Notice2Time > 0.0 && age2 >= 0.0 && age2 < 8.0) {
        float x_left = lerp(ScreenRes.x, ScreenRes.x - BOX_W - 20.0, saturate(age2 / 0.4) * saturate(age2 / 0.4) * (3.0 - 2.0 * saturate(age2 / 0.4)));
        float y_bottom = 20.0 + H_S + 5.0 + H_L + BG_PAD + 15.0 + get_y_factor(age2) * (BOX_H + 10.0);
        uint len_n2 = 0; Slot5.GetDimensions(len_n2);
        DRAW_TEXT_LINE(Slot5, len_n2, x_left + 15.0, y_bottom + (BOX_H - H_S) / 2.0, scale_S, float4(0.95, 0.95, 0.95, 1.0), saturate((8.0 - age2) / 2.0))
    }
    ...
    // Up Arrow
    if (InTri(px - ac, float2(0, 8), float2(-3.5, 3.5), float2(3.5, 3.5))) {
        return pressUp ? float4(0.3, 0.75, 1.0, 1.0) : float4(0.35, 0.35, 0.35, 1.0);
    }
    // Down Arrow
    if (InTri(px - ac, float2(0, -8), float2(-3.5, -3.5), float2(3.5, -3.5))) {
        return pressDown ? float4(0.3, 0.75, 1.0, 1.0) : float4(0.35, 0.35, 0.35, 1.0);
    }
    // Left Arrow
    if (InTri(px - ac, float2(-8, 0), float2(-3.5, -3.5), float2(-3.5, 3.5))) {
        return pressLeft ? float4(0.3, 0.75, 1.0, 1.0) : float4(0.35, 0.35, 0.35, 1.0);
    }
    // Right Arrow
    if (InTri(px - ac, float2(8, 0), float2(3.5, -3.5), float2(3.5, 3.5))) {
        return pressRight ? float4(0.3, 0.75, 1.0, 1.0) : float4(0.35, 0.35, 0.35, 1.0);
    }

    // Capture indicator dot
    if (length(px - cc) <= 4.0) {
        if (showGP) {
            return (HoveredID >= 0.0) ? float4(1.0, 0.15, 0.15, 1.0) : float4(0.25, 0.65, 1.0, 1.0); // Red (captured) / Blue (active)
        }
    ...
    // A. Main Block Panel (Gradient black with glowing red border)
    if (px.x >= main_left - BG_PAD && px.x <= right_edge + BG_PAD && px.y >= 20.0 - BG_PAD && px.y <= y_top_limit) {
        if (abs(px.x - (main_left - BG_PAD)) <= border_t || abs(px.x - (right_edge + BG_PAD)) <= border_t || 
            abs(px.y - (20.0 - BG_PAD)) <= border_t || abs(px.y - y_top_limit) <= border_t) {
            return float4(0.8, 0.15, 0.15, 0.7); // semi-transparent red border
        }
    ...
    // B. Status Block Panel (Solid black with glowing red border)
    if (px.x >= status_left && px.x <= status_right && px.y >= 20.0 - BG_PAD && px.y <= y_top_limit) {
        if (abs(px.x - status_left) <= border_t || abs(px.x - status_right) <= border_t || 
            abs(px.y - (20.0 - BG_PAD)) <= border_t || abs(px.y - y_top_limit) <= border_t) {
            return float4(0.8, 0.15, 0.15, 0.7); // semi-transparent red border
        }
    ...
    if (Notice0Time > 0.0 && age0 >= 0.0 && age0 < 8.0) {
        float x_left = lerp(ScreenRes.x, ScreenRes.x - BOX_W - 20.0, saturate(age0 / 0.4) * saturate(age0 / 0.4) * (3.0 - 2.0 * saturate(age0 / 0.4)));
        float y_bottom = 20.0 + H_S + 5.0 + H_L + BG_PAD + 15.0 + get_y_factor(age0) * (BOX_H + 10.0);
        float globalAlpha = saturate((8.0 - age0) / 2.0);

        // Life progress line (red bottom strip)
        if (px.x >= x_left && px.x <= x_left + saturate((8.0 - age0) / 8.0) * BOX_W && px.y >= y_bottom && px.y <= y_bottom + 2.0) {
            return float4(0.8, 0.15, 0.15, 0.95 * globalAlpha);
        }
        // Gradient box with black border
        if (px.x >= x_left && px.x <= x_left + BOX_W && px.y >= y_bottom && px.y <= y_bottom + BOX_H) {
            if (abs(px.x - x_left) <= border_t || abs(px.x - (x_left + BOX_W)) <= border_t || 
                abs(px.y - y_bottom) <= border_t || abs(px.y - (y_bottom + BOX_H)) <= border_t) {
                return float4(0.0, 0.0, 0.0, 0.8 * globalAlpha); // black border
            }
            float t = (px.x - x_left) / BOX_W;
            float bgA = lerp(0.85, 0.25, pow(saturate(t), 1.5)) * globalAlpha;
            return float4(0.08, 0.08, 0.08, bgA);
        }
    }
    ...
    if (Notice1Time > 0.0 && age1 >= 0.0 && age1 < 8.0) {
        float x_left = lerp(ScreenRes.x, ScreenRes.x - BOX_W - 20.0, saturate(age1 / 0.4) * saturate(age1 / 0.4) * (3.0 - 2.0 * saturate(age1 / 0.4)));
        float y_bottom = 20.0 + H_S + 5.0 + H_L + BG_PAD + 15.0 + get_y_factor(age1) * (BOX_H + 10.0);
        float globalAlpha = saturate((8.0 - age1) / 2.0);

        // Life progress line (red bottom strip)
        if (px.x >= x_left && px.x <= x_left + saturate((8.0 - age1) / 8.0) * BOX_W && px.y >= y_bottom && px.y <= y_bottom + 2.0) {
            return float4(0.8, 0.15, 0.15, 0.95 * globalAlpha);
        }
        // Gradient box with black border
        if (px.x >= x_left && px.x <= x_left + BOX_W && px.y >= y_bottom && px.y <= y_bottom + BOX_H) {
    ...
    if (Notice2Time > 0.0 && age2 >= 0.0 && age2 < 8.0) {
        float x_left = lerp(ScreenRes.x, ScreenRes.x - BOX_W - 20.0, saturate(age2 / 0.4) * saturate(age2 / 0.4) * (3.0 - 2.0 * saturate(age2 / 0.4)));
        float y_bottom = 20.0 + H_S + 5.0 + H_L + BG_PAD + 15.0 + get_y_factor(age2) * (BOX_H + 10.0);
        float globalAlpha = saturate((8.0 - age2) / 2.0);

        // Life progress line (red bottom strip)
        if (px.x >= x_left && px.x <= x_left + saturate((8.0 - age2) / 8.0) * BOX_W && px.y >= y_bottom && px.y <= y_bottom + 2.0) {
            return float4(0.8, 0.15, 0.15, 0.95 * globalAlpha);
        }
        // Gradient box with black border
        if (px.x >= x_left && px.x <= x_left + BOX_W && px.y >= y_bottom && px.y <= y_bottom + BOX_H) {
