...
class RZM_OT_validate_curve_vfx(Operator):
    ...
    def execute(self, context):
        ...
        for idx, curve_obj in enumerate(vfx_curves):
            ...
            speed = get_curve_prop(curve_obj, "speed", 0.5)
            ...
            start_radius = get_curve_prop(curve_obj, "start_radius", 0.005)
            end_radius = get_curve_prop(curve_obj, "end_radius", 0.060)
            curve_right = get_curve_prop(curve_obj, "curve_right", -0.1)
            curve_up = get_curve_prop(curve_obj, "curve_up", -0.15)
            
            # Print settings
            print(f"[RZM-VFX]   * Particle Count: {particle_count}")
            print(f"[RZM-VFX]   * Coordinate Remap Profile: {profile_raw} ({profile})")
            print(f"[RZM-VFX]   * Base Size: {mesh_fx_size_base}")
            print(f"[RZM-VFX]   * Tri Aspect: {tri_aspect}")
            print(f"[RZM-VFX]   * Speed: {speed}")
            print(f"[RZM-VFX]   * Mesh FX Type: {mesh_fx_type}")
            print(f"[RZM-VFX]   * Start Radius: {start_radius}")
            print(f"[RZM-VFX]   * End Radius: {end_radius}")
            print(f"[RZM-VFX]   * Curve Right: {curve_right}")
            print(f"[RZM-VFX]   * Curve Up: {curve_up}")
