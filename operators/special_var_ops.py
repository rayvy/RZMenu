...
class RZM_OT_UpdateShapeKey(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if hasattr(key, self.prop_name):
            ...
            try:
                if target_type is int:
                    setattr(key, self.prop_name, int(float(self.val_str)))
                elif target_type is float:
...
class RZM_OT_AddToggleDefinition(bpy.types.Operator):
    bl_idname = "rzm.add_toggle_definition"
    bl_label = "Add Toggle Definition"
    bl_options = {'REGISTER', 'UNDO'}
    def execute(self, context):
        new_def = context.scene.rzm.toggle_definitions.add()
        new_def.toggle_name = f"Toggle_{len(context.scene.rzm.toggle_definitions)}"
        context.scene.rzm_active_toggle_def_index = len(context.scene.rzm.toggle_definitions) - 1
        return {'FINISHED'}

class RZM_OT_RemoveToggleDefinition(bpy.types.Operator):
    bl_idname = "rzm.remove_toggle_definition"
    bl_label = "Remove Toggle Definition"
    bl_options = {'REGISTER', 'UNDO'}
    def execute(self, context):
        coll = context.scene.rzm.toggle_definitions
        idx = context.scene.rzm_active_toggle_def_index
        if 0 <= idx < len(coll):
            coll.remove(idx)
            context.scene.rzm_active_toggle_def_index = max(0, idx - 1)
        return {'FINISHED'}
...
classes_to_register = [
    ...
    RZM_OT_UpdateValue,
    RZM_OT_AddToggleDefinition, RZM_OT_RemoveToggleDefinition
]
